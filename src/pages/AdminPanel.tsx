    ...
    <div>
      ...
      {loading ? <div className="loading">{tt('Veriler yükleniyor...', 'Loading data...')}</div> : !stats ? <div className="error">{tt('Admin paneline erişim izniniz yok.', 'You do not have permission to access the admin panel.')}</div> : (
        <div>
          ...
          </div><div className="card" style={{ marginTop: '30px' }}><h3 style={{ color: '#d4af37' }}>🔧 {tt('Sistem Bilgisi', 'System Info')}</h3><p>API URL: {apiUrl}</p><p>Platform: Netlify + GitHub</p><p>Database: Supabase PostgreSQL</p></div></div>}
